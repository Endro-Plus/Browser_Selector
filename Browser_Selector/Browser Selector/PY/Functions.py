...
def remove_shortcut(img, main, f):
    if(not msgbox.askokcancel(title="Confirmation", message="Are you sure you want to delete this shortcut?")):
        #if they hit no, cancel
        return
    with sqlite3.connect(DB_PATH) as conn:
        cur = conn.cursor()
        cur.execute(f"SELECT browser_ID FROM browsers WHERE browser_icon = \"{img}\";")
        bid = beautify(cur.fetchall())[0]

        #turn the exe path to null
        cur.execute(f"UPDATE browsers SET browser_exe = NULL WHERE  browser_ID = {bid}")

        #remove the bpos column from the table
        cur.execute(f"DELETE FROM bpos WHERE browser_id = {bid}")

    msgbox.showinfo(title="Completed", message="Successfully deleted the shortcut")
    main(f)

def open_browser(img, main, f):
    
    
    img = img[len(str(REL_PATH))+1:]
    #check to see if we're not removing something instead
    if (btn_var.get() == 1):
        #we're deleting, not opening
        remove_shortcut(img, main, f)
        return
